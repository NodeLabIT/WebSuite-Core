...
class Login {
    ...
    static listen() {
        ...
        global.WebSuite.getWebSocketHandler().registerEvent("auto-login", (socket, data, address) => {
            global.WebSuite.getDatabase().query("SELECT COUNT(*) AS count FROM wsFailedLogins WHERE ipAddress=? AND unixtime>=? AND type='autologin'", [address, global.TimeUtil.currentTime() - 24 * 60 * 60 * 1000]).then((count) => {
    ...
    static addToBlocklist(ipAddress, autologin) {
        ...
        global.WebSuite.getDatabase().query("INSERT INTO wsFailedLogins(ipAddress, unixtime, type) VALUES (?, ?, ?)", [ipAddress, global.TimeUtil.currentTime(), type]).then(() => {
